...
export default function Modal({ children, title, onClose }: ModalProps) {
  ...
  return createPortal(
    <div
      className="bg-black/50 w-full h-screen z-[10000] fixed inset-0 flex items-center justify-center"
      onClick={onClose}
    >
      <div
        className="bg-dark-300 rounded-[10px] w-full h-full md:h-auto md:max-w-[560px] relative px-[15px] md:px-[30px] py-[35px] md:mt-10"
      ...
      >
        ...
        <div className="h-full flex flex-col md:block">
          ...
          <div className="flex-1 flex flex-col justify-center items-center md:block">
            {/* 모바일 타이틀 */}
            <h2 className="text-xl font-bold text-center mb-[20px] md:hidden">{title}</h2>
